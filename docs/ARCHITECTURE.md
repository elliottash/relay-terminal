...
| Ctrl+Shift+G (`program.delegate`), the banner button, the palette | Hands the running program to the agent (section 9.2). With text in the prompt box it sends that request too. Pressed again, or Ctrl+H, takes it back |
...
over. `Ctrl+Shift+G` (`program.delegate`), the banner's "Let the agent drive" button and the
