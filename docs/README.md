...
| [GLOBAL-PROJECT-BOARD-RESEARCH.md](GLOBAL-PROJECT-BOARD-RESEARCH.md) | A global project board above the per-project Boards (#V3R3): what the owner's Trello, Google Sheet and `~/project-notes` trackers hold and why each rotted, what fourteen products and fourteen file-based systems agree the layer above a project is, and a proposal — a `project` card type in the global Board with criticality, time and agent budgets, ball-in-court, read-time rollups from the per-project boards, one-time imports and a weekly review skill. The three source reports are under `research/global-project-board/` |
