...
for line in sys.stdin:
    ...
    if kind == 'presets':
        ...
        if failure == 'always' or (failure == 'once' and not marker.exists()):
            ...
            time.sleep(.3)
            emit(event='configured',model='gpt-6-astra',mode='build',agent_role='main',effort='high',session_id='fixture',roles={'main':{'model':'gpt-6-astra'}})
    elif kind == 'set_mode':
...
if __name__=='__main__':
    run_case('plan-first','none',1,'plan')
    run_case('toggle-back','none',2,'build')
    run_case('recover-first','once',1,'plan')
    run_case('retry-button','always',1,'plan')
