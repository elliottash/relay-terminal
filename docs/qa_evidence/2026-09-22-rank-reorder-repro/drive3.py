#!/usr/bin/env python3
"""Reproduce part 3: drag with a filter typed. Isolated env, seeded tier lists."""
import os, pathlib, subprocess as sp, tempfile, time
ROOT = pathlib.Path(__file__).resolve().parents[3]
OUT = pathlib.Path(__file__).resolve().parent
QA = pathlib.Path(tempfile.mkdtemp(prefix='relay-rank-filter-'))
print(QA, flush=True)
...
try:
    ...
    key('ctrl+shift+m')
    time.sleep(1)
    run('xdotool','type','--delay','60','claude-m')   # filter the list
    time.sleep(1.5)
    sections, models = rows(shot('f1-filtered.png'))
    print('sections:', sections, 'model row tops:', models, flush=True)
    main_y, flash_y = sections.get('main'), sections.get('flash')
    main_rows = [y for y in models if main_y and y > main_y and (not flash_y or y < flash_y)]
    print('main rows under filter:', main_rows, flush=True)
    if len(main_rows) >= 2:
        drag(450, main_rows[0]+8, 450, main_rows[1]+18)
        shot('f2-after-drag.png')
    time.sleep(1)
finally:
