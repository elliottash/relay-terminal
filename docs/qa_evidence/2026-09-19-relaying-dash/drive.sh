#!/usr/bin/env bash
# Card #RR0G, a dash after "Relaying": implementer screenshots under
...
#
#   docs/qa_evidence/2026-09-19-relaying-dash/drive.sh [build-dir] [scene...]
#
# Scenes (all by default), each into implementer-<scene>-x.png (full window) and
# implementer-<scene>-x-comp.png (the composer, bottom 240 px, at 100 % and 200 % in
# -comp2x.png for reading). Every one of them is a spelling of the busy line, and what to
# look at is the spaced en dash after "Relaying":
...
scenes=${*:-relaying running spawn question}
width=1440 height=900
port=${RELAY_QA_PORT:-8815}

display=
for n in $(seq 160 199); do [[ -e /tmp/.X11-unix/X$n ]] || { display=:$n; break; }; done
[[ -z $display ]] && { echo "no free display"; exit 1; }

sandbox=$(mktemp -d /tmp/relay-rr0g.XXXXXX)
...
wait_ready() {   # wait_ready <scene>
    ...
    while ((tries < 20)); do
        ...
        sleep 2
        probe=$(mktemp /tmp/rr0g-probe.XXXXXX.png)
