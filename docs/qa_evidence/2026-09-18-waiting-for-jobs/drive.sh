#!/usr/bin/env bash
# "waiting for N jobs . . ." in the prompt box (card #KP4M; feature intake 2026-09-18: "if a session
# is waiting for background jobs, indicate \"waiting for jobs...\" similar to \"waiting for
# subagents...\"."). The same surface and machinery as card #V7QD.
#
#   docs/qa_evidence/2026-09-18-waiting-for-jobs/drive.sh [build-dir]
#
...
#
#   01  blocked on command_output: the prompt box says "waiting for 1 job", the strip clock too
#   02  ~0.7 s later: the dots are at another phase (the same line, animated)
#   03  typing a steer: the line is gone from under the caret, nothing obstructs the text
#   04  the job finished and the turn answered: the ordinary placeholder is back
#   05  the other way in — the turn ended and one background job is still running
#   06  that job finished: the ordinary placeholder is back
#   07  a subagent and a job at once: "waiting for 1 subagent, 1 job . . ." — one line, both named
#
...
root=$(cd ../../.. && pwd)
build=${1:-$root/build}
width=1400 height=900 port=$((18800 + $$ % 90))
...
sandbox=${TMPDIR:-/tmp}/relay-waiting-jobs-$$
...
# ----- blocked on command_output ----------------------------------------------------------------
click 600 825
t '*hold for the job'; k Return; sleep 3
# The provider dialog on first use: tick the consent box, then Save.
click 384 615; sleep 0.3; click 862 647; sleep 9
shot 01-blocked-on-command-output
sleep 0.7
shot 02-the-dots-at-another-phase
t 'and tell me what it printed'; sleep 0.6
shot 03-a-steer-is-never-obstructed
for _ in $(seq 30); do k BackSpace; done; sleep 20
shot 04-the-job-came-back-the-ordinary-placeholder

# ----- the turn ends, a background job is still running -------------------------------------------
t '*leave it running'; k Return; sleep 10
shot 05-the-turn-ended-one-job-left
sleep 32
shot 06-that-job-finished-too

# ----- a subagent and a job at once ---------------------------------------------------------------
t '*both at once'; k Return; sleep 14
shot 07-one-subagent-and-one-job
