#!/usr/bin/env bash
# Ground truth for #BXCN's live check: the same driven sequence as drive.sh, but instead of
# decoding pixels it reads the layout Relay itself persists — every splitter move schedules a
# save of $XDG_DATA_HOME/relay/state/windows.json, and every node there carries its "split"
# orientation and "sizes". The board pane's actual width after Ctrl+Alt+0 and after each
# Execute (`x`) is read from the app's own record, dumped after every step.
#   ground-truth.sh [build-dir] [out-dir]
set -uo pipefail
...
sandbox=$(mktemp -d /tmp/rl-bxcn-gt.XXXX)
xvfb_pid= relay_pid=
cleanup() {
    local pid
...
# No WM under Xvfb: a resize can silently not take. Make sure it did.
for attempt in 1 2 3; do
    eval "$(xdotool getwindowgeometry --shell "$win")"
    (( WIDTH >= width - 2 )) && break
    xdotool windowsize "$win" "$width" "$height"; sleep 2
done
echo "window ${WIDTH}x${HEIGHT} (wanted ${width}x${height})" >>"$out/ground-truth.txt"
sleep 4
...
click_word() {
    ...
    [[ -z $box ]] && { echo "MISSING WORD: $2" >>"$out/ground-truth.txt"; return 1; }
    xdotool mousemove $(( ${box% *} + ${3:-0} )) $(( ${box#* } + ${4:-0} )) click 1
    sleep 1.5
}
dump_layout() {  # tag
    sleep 1   # let the debounced save land
    python3 - "$XDG_DATA_HOME/relay/state/windows.json" "$1" "$out/ground-truth.txt" <<'PY'
import json, sys
...
def walk(node, depth=0):
    ...
    if "split" in node:
        ...
        pane = node.get("pane", node.get("board", {}))
        label = "BOARD" if "board" in node else ("pane" if "pane" in node else keys)
        if isinstance(pane, dict):
            label += f" cwd={pane.get('cwd', '')}"
...
: >"$out/ground-truth.txt"
echo "fixture card: $card_id" >>"$out/ground-truth.txt"

xdotool key --window "$win" ctrl+shift+s; sleep 8
import -window root "$out/gt-01-board-open.png"
click_word "$out/gt-01-board-open.png" "READY" 25 0 || true; sleep 2
import -window root "$out/gt-02-unfolded.png"
click_word "$out/gt-02-unfolded.png" "Alpha" 0 0 || true; sleep 2.5
dump_layout "board open, card open"

xdotool key --window "$win" ctrl+e; sleep 3
xdotool key --window "$win" ctrl+e; sleep 3
dump_layout "after two Ctrl+E"

xdotool key --window "$win" ctrl+alt+0; sleep 3
import -window root "$out/gt-03-equalized.png"
dump_layout "after Ctrl+Alt+0 (equalize)"

click_word "$out/gt-03-equalized.png" "THREAD" 0 0 || true; sleep 1.5
xdotool key --window "$win" x; sleep 9
import -window root "$out/gt-04-executed.png"
dump_layout "after first Execute (x)"

click_word "$out/gt-04-executed.png" "THREAD" 0 0 || true; sleep 1.5
xdotool key --window "$win" x; sleep 9
import -window root "$out/gt-05-executed-twice.png"
dump_layout "after second Execute (x)"

if kill -0 "$relay_pid" 2>/dev/null; then echo "relay alive at the end: yes" >>"$out/ground-truth.txt"; else echo "relay alive at the end: NO" >>"$out/ground-truth.txt"; fi
[[ -s "$sandbox/relay.log" ]] && cp "$sandbox/relay.log" "$out/ground-truth-relay.log"
echo "ground truth in $out/ground-truth.txt"
