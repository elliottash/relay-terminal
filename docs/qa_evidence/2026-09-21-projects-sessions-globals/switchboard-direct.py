...
try:
    ...
    time.sleep(2)
    key('ctrl+shift+s'); time.sleep(6); shot('07-switchboard-direct.png')
    for folder in ('issues', '.switchboard', 'switchboard', '.git'):
        assert not (loose/folder).exists(), 'Opening Switchboard wrote '+folder
    worker_log = QA/'data/relay/logs/worker.log'
    assert not worker_log.exists() or 'protocol_error' not in worker_log.read_text(), 'Switchboard failed to load'
    # Toggle closes the board; project management remains separately reachable.
    key('ctrl+shift+s'); key('ctrl+shift+p')
    print('PASS: Switchboard opens directly in a loose folder without creating board/git files.', flush=True)
finally:
