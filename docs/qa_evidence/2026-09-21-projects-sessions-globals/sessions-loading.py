...
try:
    ...
    time.sleep(2)
    key('ctrl+shift+p')
    # Mouse entry is the regression: no shortcut-triggered refresh may mask it.
    run('xdotool','mousemove','811','99','click','1'); time.sleep(4)
    shot('08-sessions-loaded.png')
    run('xdotool','mousemove','1300','140','click','1'); time.sleep(1)
    shot('09-closed-button.png')
    run('xdotool','mousemove','800','140','click','1'); time.sleep(1)
    # The saved fixture must remain indexed; the two screenshots verify rendered navigation.
    import sqlite3
    db=sqlite3.connect(QA/'data/relay/index.db')
    assert db.execute("select count(*) from conversations where title='Session loading fixture'").fetchone()[0] == 1
    print('PASS: seeded session indexed; captured mouse-loaded Sessions and nested Recently closed.', flush=True)
finally:
