...
out=/tmp/mdl1verify-artifacts/extra
build=${RELAY_BUILD:-$root/build}
width=1600 height=900

mkdir -p "$out"
display=
for n in $(seq 900 920); do [[ -e /tmp/.X11-unix/X$n ]] || { display=:$n; break; }; done
[[ -z $display ]] && { echo "no free display in 800..830"; exit 1; }
echo "display $display"

sandbox=/tmp/mv-extra          # short: XDG_RUNTIME_DIR holds a unix socket
xvfb_pid= relay_pid=
...
export RELAY_GLM_API_KEY=xvfb-not-a-real-key
