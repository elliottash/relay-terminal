...
out=/tmp/mdl1verify-artifacts/fresh
build=${RELAY_BUILD:-$root/build}
width=1600 height=900

mkdir -p "$out"
display=
for n in $(seq 940 950); do [[ -e /tmp/.X11-unix/X$n ]] || { display=:$n; break; }; done
[[ -z $display ]] && { echo "no free display in 720..750"; exit 1; }
echo "display $display"

sandbox=/tmp/mv-fresh           # short: XDG_RUNTIME_DIR holds a unix socket
xvfb_pid= relay_pid=
...
printf '#!/bin/sh\nexit 1\n' >"$sandbox/bin/codex"
printf '#!/bin/sh\nexit 1\n' >"$sandbox/bin/claude"
chmod +x "$sandbox/bin/codex" "$sandbox/bin/claude"
