...
The real application was driven under Xvfb with temporary XDG directories and a fake worker (`drive.py`). Two turns were printed, the second was selected in the Rewind picker, and the resulting notice was clicked with the mouse. No provider calls or user settings were used. After collapse, an idle shell receives an empty input line to establish a fresh Readline prompt before opening the adjacent pane resizes it.
