...
"""Loopback-only OpenAI-compatible endpoint for the #0STR pane-header screenshots.

    python3 stub-provider.py 8815

Copied from the #HQ2B run (via #4E13); what the user has said in this conversation decides
the turn:
...
def user_text(request):
    # EVERY user message, not just the first. The driver's readiness probe (`echo relayqaready`)
    # reaches the agent in this sandbox, so it is the first user message of the conversation and
    # keying on that one made every scene take the 4 s "Done." branch. The scene keywords are
    # unique enough that "somewhere in what the user said" is the right test — and the worker
    # appends a user-role message of its own at the end of a turn, so keying on the LAST one is
    # just as wrong (docs/qa_evidence/2026-09-19-helpful-line-breaks).
    parts = []
