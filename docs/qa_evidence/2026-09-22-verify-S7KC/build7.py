...
OUT=Path(__file__).resolve().parent / "build7"
...
with tempfile.TemporaryDirectory(prefix='relay-ssh-audit-') as tmp:
 ...
 try:
  ...
  with (OUT/'gui.log').open('w') as log:
   ...
   (p/'remote/bin').mkdir()
   (p/'remote/bin/remote_only_probe').write_text('#!/bin/sh\necho REMOTE-PROBE\n')
   (p/'remote/bin/remote_only_probe').chmod(0o755)
   send('!export PATH=' + str(p/'remote/bin') + ':$PATH')
   x('type','remote_only_p');x('key','Tab');time.sleep(2);shot('13-remote-command-completion');x('key','ctrl+a','BackSpace')
   send('!sleep 20',1);send('!echo CANCELLED_MUST_NOT_RUN',.5);shot('14-cancel-queued')
   x('mousemove','--window',win,'1165','742');x('click','1');time.sleep(.3)
   x('key','ctrl+h');time.sleep(.3);x('key','ctrl+c');time.sleep(2);x('key','ctrl+shift+h');time.sleep(1);shot('15-after-cancel-interrupt')
   for socket in (p/'run/relay-ssh').glob('*'):
    hidden=socket.with_name(socket.name+'.unavailable');socket.rename(hidden)
    try:
     x('type','cat LOCAL_ONLY_SSH_AU');x('key','Tab');time.sleep(2);shot('16-missing-socket-lookup');x('key','Escape','ctrl+a','BackSpace')
    finally:hidden.rename(socket)
   ...
   x('type','cat LOCAL_ONLY_SSH_AU');x('key','Tab');time.sleep(2);shot('17-local-completion-restored')
