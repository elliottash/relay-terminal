...
OUT=Path(__file__).resolve().parent / "zsh-trial"
...
with tempfile.TemporaryDirectory(prefix='relay-ssh-audit-') as tmp:
 ...
 for n in ['scripts','assets']:(p/'fixture'/n).symlink_to(ROOT/n)
 shutil.copytree(ROOT/'shell',p/'fixture/shell')
 remote_script=p/'fixture/shell/remote-integration.sh'
 old='__relay_r_redraw() { __relay_r_rows=$(__relay_r_rows_for "$BUFFER"); }'
 new='__relay_r_redraw() { if [ -n "$BUFFER" ]; then __relay_r_rows=$(__relay_r_rows_for "$BUFFER"); else printf "\\n\\n"; zle reset-prompt; fi; }'
 remote_script.write_text(remote_script.read_text().replace(old,new))
