...
OUT=Path(__file__).resolve().parent / "delayed7"
...
with tempfile.TemporaryDirectory(prefix='relay-ssh-audit-') as tmp:
 ...
 (p/'shim').mkdir()
 shim=p/'shim/ssh'
 shim_text=('#!/usr/bin/python3\nimport sys,os,time\nif any("compgen" in arg for arg in sys.argv):\n with open('+repr(str(OUT/'delayed-calls.txt'))+',"a") as f:f.write("delayed completion\\n")\n time.sleep(2)\nos.execv("/usr/bin/ssh",["ssh",*sys.argv[1:]])\n')
 shim.symlink_to('/usr/bin/ssh')
 env['PATH']=str(p/'shim')+':'+env['PATH']
 ...
 try:
  ...
  with (OUT/'gui.log').open('w') as log:
   ...
   shim.unlink();shim.write_text(shim_text);shim.chmod(0o755)
   x('type','printen');x('key','Tab');time.sleep(3);shot('01-command-completion');x('key','ctrl+a','BackSpace')
   x('type','cat REMOTE_ON');x('key','Tab');time.sleep(.3);x('key','ctrl+a');x('type','UNCHANGED_DRAFT');time.sleep(3);shot('02-stale-edit');x('key','ctrl+a','BackSpace')
   x('type','cat REMOTE_ON');x('key','Tab');time.sleep(.3);x('key','ctrl+h');time.sleep(.3);x('type','exit');x('key','Return');time.sleep(3);x('key','ctrl+shift+h');time.sleep(1);shot('03-stale-disconnect')
