...
OUT=Path(__file__).resolve().parent / "fullscreen"
...
with tempfile.TemporaryDirectory(prefix='relay-ssh-audit-') as tmp:
 ...
 try:
  ...
  with (OUT/'gui.log').open('w') as log:
   ...
   send("!printf '\\033[?1049h\\033[HSCREEN-PROOF'; read -rsn1; printf '\\033[?1049l'",2);shot('01-fullscreen')
   x('key','ctrl+h');time.sleep(.5);x('type','q');time.sleep(3);x('key','ctrl+shift+h');time.sleep(1);shot('02-fullscreen-return')
   send('!printf \"AFTER-SCREEN\\n\"');shot('03-after-screen-command')
   send('!exit',2)
