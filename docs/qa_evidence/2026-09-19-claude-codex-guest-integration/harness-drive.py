...
turn on the owner's subscription; the prompt asks for one word.
    ...
    xvfb-run -a -s "-screen 0 1600x1000x24" python3 harness-drive.py <output-dir>
...
def main() -> int:
    ...
    if not shutil.which("claude"):
        log("no real claude on PATH: this run needs one")
        return 1
    ...
    try:
        ...
        text = screenshot(output / "implementer-harness-01-before.png")
        ...
        while time.monotonic() < deadline and not configured:
            time.sleep(2)
            text = screenshot(output / "implementer-harness-02-picked.png")
            configured = "this pane's agent" in text or "Claude Code" in text
        ok &= report("02-picked", text, ["Claude Code"], ["fake-claude", "--settings"])
        ...
        while time.monotonic() < deadline and not answered:
            time.sleep(3)
            text = screenshot(output / "implementer-harness-03-answered.png")
            lowered = text.lower()
        ...
        time.sleep(4)
        text = screenshot(output / "implementer-harness-04-shell.png")
        ...
        (output / "harness-run.log").write_text(out, encoding="utf-8")
        log(f"relay output saved to {output / 'harness-run.log'}")
