#!/usr/bin/env bash
# A label hashtag copies when clicked, and a `#ID` reference zooms (card #3ZAP: "in the
# switchboard, make it where if you click on hash tags, it copies them to c…", refined by the
# owner: the copy lives on the label surfaces — the list-row badges, the card-top meta labels,
# and `#tag` in the card's body/thread — while `#ID` references zoom to the card they name).
#
...
#
#   docs/qa_evidence/2026-09-20-switchboard-hashtag-copy/drive.sh [build-dir] [out-dir]
set -uo pipefail
root=/home/elliott/repos/relay-terminal
build=${1:-$root/build}
out=${2:-$root/docs/qa_evidence/2026-09-20-switchboard-hashtag-copy}
...
pos=$(bug_on_line "$out/03-bravo-open.png" "labels")
...
pos=$(bug_on_line "$out/03-bravo-open.png" "missing")
...
pos=$(bug_on_line "$out/03-bravo-open.png" "Also")
echo "06 thread #bug at: $pos" >>"$out/ocr.txt"
click_at ${pos% *} ${pos#* }
shot 06-thread-click
