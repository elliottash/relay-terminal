...
for line in sys.stdin:
    ...
    if kind == 'presets':
        ...
        emit(event='started',id=r.get('id')); emit(event='done',id=r.get('id')); emit(event='agent_finished',id=r.get('id'),outcome='done')
'''
...
''')
        ...
        try:
            ...
            with (OUT/(name+'-stderr.log')).open('w') as err:
                ...
                for prompt in ['ls','/deliver','ls','/clean-commit fix this','ls','/local-model-setup','ls','/skill deliver fix this','ls','/compact']:
                    x('key','--clearmodifiers','ctrl+a')
                    x('type','--clearmodifiers','--delay','20',prompt)
                    x('key','--clearmodifiers','Escape')
                    time.sleep(.8)
                    label=prompt.replace('/','').replace(' ','-')
                    subprocess.run(['import','-window',win,str(OUT/(label+'.png'))],env=env,check=True)
                x('key','--clearmodifiers','ctrl+a')
                x('type','--clearmodifiers','--delay','20','/deliver fix this')
                x('key','--clearmodifiers','Escape')
                time.sleep(.5)
                x('key','--clearmodifiers','Return')
                time.sleep(.5)
                x('key','--clearmodifiers','Return')
                wait_for(lambda: len(commands('ask'))>=2,'skill did not submit as agent')
                assert commands('ask')[-1]['request']['skills']==['deliver'],commands('ask')[-1]
                (OUT/'requests.jsonl').write_text((home/'requests.jsonl').read_text())
                print('PASS: /deliver Enter submits an agent ask with skills=[deliver]',flush=True)
        finally:
