...
out=Path(__file__).resolve().parent; gate=Path('/tmp/claude-1000/land/tooling-repair/verify')
root=Path(tempfile.mkdtemp(prefix='v7bm4-repair-')); (root/'board/features').mkdir(parents=True)
...
card.write_text(card.read_text().split('## Tests')[0]+'## Plan\n'+('Long plan text. '*2000)+'\n## Tests\n'+''.join(f'- `tests/test_missing_{i}.py`\n' for i in range(20))+'\n## Execution Summary\nTail marker retained.\n')
...
try:
 ...
 assert 'Tail marker retained.' in drive('read','sections')['text']
 line=drive('read','boardTestsLine');assert '20 listed' in line['text'],line
