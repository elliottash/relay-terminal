#!/usr/bin/env bash
# A ⧉ beside a card's `#ID` copies the reference (card #FT77: "in the switchboard, in the issue
# rows … or the card headers … put the copy icon button next to it, you click on that and it will
# copy the hash tag", with screenshots of the `#DC4J` row and the card page's header). The button
# lives beside the id on every card row and beside the ref on the card page; a click puts `#ID`
# on the clipboard, with the notice and the toast #Y2F4 added, and the row is never selected by
# it. This drive re-runs #3ZAP's surfaces and clicks the two new buttons, on a fixture whose card
# ids are pinned to the owner's own: the reference is `#KAN3`, the card under test `#DC4J`.
#
...
build=${1:-$root/build}
out=${2:-$root/docs/qa_evidence/2026-09-21-switchboard-ref-copy-button}
width=1440 height=900
mkdir -p "$out"

display=
# A display is free when nobody's socket file is there AND no Xvfb still runs on it: a server
# whose socket file was deleted from under it leaves the display taken with nothing to show for it.
for n in $(seq 170 199); do
    [[ -e /tmp/.X11-unix/X$n ]] && continue
    pgrep -f "Xvfb :$n " >/dev/null 2>&1 && continue
    display=:$n; break
done
...
id_box_top() {  # image -> "left top width height" of the topmost `#DC4J` token. The OCR reads
                # the id's glyphs loosely: `#DC4J` has come back as `#0C4J`, `#0c4]`.
    words "$1" | awk -v ox="$board_x" '{w=$12; sub(/^#/, "", w)}
        tolower(w) ~ /^[d0]c4/ {print int($7/2)+ox, int($8/2), int($9/2), int($10/2); exit}'
}
...
# 2b. The ⧉ beside the row's `#ID` (#FT77): a click copies #DC4J, notice and toast, without
#     opening the card (the shot still shows the list). The glyph sits straight after the id's
#     own glyphs, so the click goes a little right of the OCR's DC4J box.
# The glyph ends a few pixels before the title's column, so the click goes just left of the
# title — OCR merges the ⧉ into the id token ("#0C4Jo0"), which makes the id's own box useless.
box=$(word_box "$out/01-board-open.png" "Bravo")
bx=$(( $(echo $box | cut -d' ' -f1) - 16 ))
by=$(( $(echo $box | cut -d' ' -f2) + $(echo $box | cut -d' ' -f4) / 2 ))
echo "02b row copy-glyph at: $bx $by" >>"$out/ocr.txt"
clip_clear
toast_shot 02b-row-copy "$bx" "$by"
notice_has "02b row ⧉ notice" "$out/02b-row-copy.png" "Copied #"
toast_has "02b row ⧉ toast" "$out/02b-row-copy.png" "#DC4J copied"
check "02b row ⧉ clipboard" "#DC4J" "$(clip)"
check "02b list still shown (card not opened)" "yes" "$(words "$out/02b-row-copy.png" | grep -q Bravo && echo yes)"
...
# 4b. The ⧉ beside the card page's `#ID` (#FT77): the same copy from the page's own header. The
#     button follows the ref label with the header's 6 px spacing, so the click goes a little
#     further right of the OCR's DC4J box than the row's painted glyph does.
box=$(id_box_top "$out/03-bravo-open.png")
bx=$(( $(echo $box | cut -d' ' -f1) + $(echo $box | cut -d' ' -f3) + 23 ))
by=$(( $(echo $box | cut -d' ' -f2) + $(echo $box | cut -d' ' -f4) / 2 ))
echo "04b header copy-button at: $bx $by" >>"$out/ocr.txt"
clip_clear
toast_shot 04b-header-copy "$bx" "$by"
notice_has "04b header ⧉ notice" "$out/04b-header-copy.png" "Copied #"
toast_has "04b header ⧉ toast" "$out/04b-header-copy.png" "#DC4J copied"
check "04b header ⧉ clipboard" "#DC4J" "$(clip)"
