...
**Since 2026-09-20 the live inventory is the Test suites pane** (card #7BM4, Board tool
...
| `tests/test_board.py` | 92 | Board format tests: cards, ids, ranks, task markers, threads, check, migrate |
| `tests/test_board_import.py` | 50 | `relay_core.board_import`: proposing cards from a tracker, and writing the accepted ones |
| `tests/test_board_protocol.py` | 111 | Board worker protocol (docs/AGENT-SESSIONS-PROTOCOL.md section 17) and the `board_*` tools' wiring into the agent |
| `tests/test_board_tools.py` | 125 | Board agent tools: the six `board_*` tools, their refusals and their guardrails |
...
| `tests/test_forge_sync.py` | 106 | Two-way sync between Board cards and GitHub issues (`#GDQN`) |
...
| `tests/boardmodel_test.cpp` | 43 | The Board pane's pure logic: which section a card falls into, the filter language, the ordering, the row list the one scrolling view draws, and the `#` picker's ranking. No worker, no files, no network |
| `tests/boardworkspace_test.cpp` | 8 | Which project's Board a pane is looking at (src/BoardWorkspace.h). The Board is per project, so the rule has to answer from the candidate directories it is handed and from nothing else — in particular never from the directory Relay itself was started in |
...
| `tests/outputlinks_test.cpp` | 29 | Clickable paths (#YZTK), card references (Board design section 5) and the keyboard walk over them (#GWXM): which spans of a line of terminal output are links, what they resolve to against a pane's working directory and card index, and how the keyboard cursor moves over the ordered list |
...
| `tests/projectinit_test.cpp` | 19 | "Initialize a project and create a Board here?" (src/ProjectInit.h, protocol 19.12/19.13). The owner's rule of 2026-09-18 is that `<project>/board/` appears only after one yes, and that only five acts may ask |
| `tests/projects_test.cpp` | 28 | The project model behind the per-project Board (src/Projects.h): the candidate project of a terminal directory, the project key that has to match the backend's workspace digest, which board a project's cards go to (and whether the user has to be asked first) |
...
| `tests/sharingpane_test.cpp` | 16 | The multiplayer model behind the Sharing pane (#W5N2, docs/REMOTE-PROTOCOL.md section 10), and the view's two pages (#SMDX). The model half is the logic the owner's answers depend on — who is here, what is waiting, whose countdown has run out, what the pane header says. The view half is Devices and People as built: the default page, the empty People state, the invite form's scope picker, a pairing offer's lifecycle (started only by "Add a device…", revoked on leaving the page or Done), the approval card with Refuse first, knocks under "Waiting for you", and the scope headings over "Shared now". All of it runs without a hub and without a display |
