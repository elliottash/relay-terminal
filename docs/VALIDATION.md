...
| `engine/tests/FaintInkTest.cpp` | 15 | Faint ink (#N50J, `engine/view/FaintInk.h`): WCAG luminance and contrast, the flat 40% fade being under the floor on both shipped themes' muted grey, and the helper fading as far as it can while staying at 4.5:1 — unchanged where there is no room, and never fainter than the old alpha |
