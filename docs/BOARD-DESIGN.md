...
- **Not in git, and not a file per fault.** A signal is the fold of two append-only files under the
  board's private root (`BOARD-FORMAT.md` §5.1): every test execution, and every action a
...
- **What Discuss writes to the thread.** The worker writes both ends, as it always has: the owner's
  ...
  entries themselves did not change a byte — `BOARD-FORMAT.md` 3.
...
[`BOARD-TOOLING-RESEARCH.md`](BOARD-TOOLING-RESEARCH.md)): the Board is not only
...
- **`## Tests` on a card, and Check.** A Markdown list, one line per test — an invocation and an
  ...
  are in [`BOARD-FORMAT.md`](BOARD-FORMAT.md) §2.6. The card page shows a strip above
...
- Creation warnings: the sixth card in a turn and the 31st in a rolling hour per workspace
  succeed, but warn the person in the Board activity toast and the agent in the tool result.
  Later creates continue. Other writes remain capped at 100 per turn and return
  `board_rate_limited` at that limit. Writes are atomic and hash-checked like `write_file`.
