...
[`BOARD-DESIGN.md`](BOARD-DESIGN.md) and [`TASKS-AND-MEMORY-DESIGN.md`](TASKS-AND-MEMORY-DESIGN.md).
...
issues/cases.jsonl             the case ledger (#95VZ): one JSON line per served case, append-only,
                               committed; `merge=union` like the threads (protocol 19.22)
...
- `ai_may_gate_after:` and `sample_after:` `never` (default), `always`, or a number of verified
  cases on record before `ai-text` / `ai-visual` may be the `primary` rung, or a `sample` line may
  stand. The number is counted from the board's case ledger, `cases.jsonl` (#95VZ, protocol
  19.22): rows whose `verdict.result` is `pass` for the server that serves the card — the skill
  the turn loaded, or the server the ledger already names for that card — and, when no row
  says which server that is, passing rows across the board. A board with no ledger counts 0,
  so a number reads as "not yet" until cases have passed. AI gating also needs the card's `qa`
  block to name a verifier outside the author's lineage.
...
**The case ledger (#95VZ).** `issues/cases.jsonl` is one JSON line per served case, appended
under the board directory's lock and never rewritten: `id`, `when`, `server` (a skill id, a
program path, or `person`), `server_version`, `served_by` (`person` or a model signature),
`card` (optional), `input` (a path or a one-line reference, never content; absent on a row a
`confidential: yes` profile wrote), `cost` (`tokens`, `seconds`, `money`), `signal` (the verify
primary mode and its result), `escalated`, `verdict` (`pass` | `fail` | `pending`, who, revision)
and `confidential`. The worker writes rows at the end of a turn that loaded a profiled skill, on
a Try it answer, on a `## Verdict`, on a move to `done` (pass) or back a stage (fail), and on the
agent's `board_case` tool; `board_list {cases: true}` reads them back, and a pane whose
workspace is not this board's project never receives the confidential rows. Protocol 19.22 has
the shape and an example row. Commit it with the cards: it is the record of what was served.
