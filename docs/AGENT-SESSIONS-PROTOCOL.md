...
| `app_action_list` | `search?` | the action catalog, at most 60 entries, `agent_safe` on each row and `runnable` counting them, so the agent can name an action it may not run and tell the person where the button is |
