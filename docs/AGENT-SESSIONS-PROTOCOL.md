...
- `set_model {preset, base_url, model, extra, max_tokens, context_window, use_stored_key, api_key?}` → swaps the provider, keeping the conversation. Event `model_changed {model, model_name, preset, context_window, effort, applies, in_flight_model?, in_flight_model_name?, will_compact?}`, or `model_switch_refused` (below).
  ...
  - **`when`: where a mid-turn switch lands** (card #7QH0, 2026-09-25). `set_model {…, when?: "steer"|"queue"|"now"}`. `"steer"` — and an absent `when`, so older clients are unchanged — is the next step boundary described above. `"queue"` holds the switch to the end of the running turn, which finishes whole on the model it started on (`applies: "turn_end"`, `model_applied {at: "turn_end"}` after `agent_finished`); a held switch never pre-empts a retry wait (#DC4J). `"now"` holds it the same way and stops the running turn for it (`interrupting: true`): the turn ends `cancelled`, the switch lands as it ends, and — unlike `cancel` — the worker's queue is **not** paused, so what waits behind the turn runs on the new model. An exclusive task (a compaction) is not stopped. Mid-turn, `model_changed` echoes `when`. Idle, every `when` applies at once as before. The desktop pane sends none of these for an idle pick; for a pick made while its agent works it queues a `/model <name>` row in its own queue, sends it idle when its turn comes, and walks it up the ladder (`"steer"`, then `"now"`) on Enter.
  - `model_withdraw {id?}` takes back a switch that has not landed (the × on a steered `/model` row): `model_switch_withdrawn {id, withdrawn: true, model, current_model, preset, context_window, effort}` names the switch dropped and the model in force, for the chip to go back to. `withdrawn: false` (and nothing else) when there was nothing left to take back — it landed, or it is compacting on its way in.
  ...
  - **Above 128K tokens: compacted on any model change** (#SWCP, owner 2026-09-23: "when changing between models, i would say, compact if there are more than 128K tokens in the context"). A conversation over `SWITCH_COMPACT_TOKENS` (128,000) is compacted before the switch even when the new window holds it, towards `min(the new window's limit, 128,000)`, through the same path and events as above (`will_compact: true`, `compaction_started/compacted {reason: "model_switch"}`, `model_applied {compacted: true}`). This one is a preference, not a fit: when that compaction fails — typically a guest harness in force with no `summaries` role, which cannot summarise — the switch lands with the whole conversation and a `status` says so, where a compaction needed to fit the window would refuse the switch. A stop still keeps the old model.
...
case in section 8 the GUI derives it, and shows neutral "Relaying · waiting for N jobs . . ."
immediately above the prompt frame (#R3YN), leaving the editor's placeholder unchanged. It is
