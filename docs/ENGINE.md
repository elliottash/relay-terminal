...
| Display | `writeToDisplay(bytes)` (inline output, never reaches the program), `redrawPrompt()` (sends Ctrl+X Ctrl+P by default, Relay's Bash binding), `setRedrawPromptSequence()` |
