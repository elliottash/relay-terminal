...
Add, remove, merge and rename are one write of this file, from any of three places: the gear at the
end of the section checkboxes in the Switchboard pane, the agent's `board_sections` tool during a
cleanup run, or your editor. The first two go through one validator and land in the write log, so
`board_undo` takes a section change back like any other write.
