...
class CatalogueTests(unittest.TestCase):
    ...
    def test_a_codex_that_fails_or_is_missing_gets_the_fallback_menu(self):
        ...
        # codex's own default for Main — `medium` for gpt-5.6-terra, never its top level `ultra`
        # — and `xhigh` for High, the owner's rule for a plan turn. gpt-6-astra, gpt-6-sol and
        # gpt-6-luna have rows; terra does not.
        self.assertEqual({m["id"]: m["tier_effort"] for m in rows["guest:codex"]["models"]},
                         {"gpt-6-astra": {"main": "medium", "high": "xhigh", "flash": "low", "lite": "low"},
                          "gpt-6-sol": {"main": "medium", "high": "xhigh", "flash": "low", "lite": "low"},
                          "gpt-5.6-terra": {"main": "medium", "high": "xhigh", "flash": "low", "lite": "low"},
                          "gpt-6-luna": {"main": "high", "high": "max", "flash": "low", "lite": "low"}})
        self.assertEqual(rows["guest:codex"]["models"][1]["default_effort"], "medium")
        ghp.reset_catalog()
        ghp._detected = None
