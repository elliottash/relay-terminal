...
session: b0371114-fc81-45bb-983f-cc6f9a5ded6a
rank: zzzzzzzzzzzzzzzzzzzr
created: '2026-09-24'
verify: {artifact: visual, primary: script, also: [ai-visual, person], human: optional, criteria: 'Open the Sharing pane: Devices shows the switch, the address, the paired devices and Add a device with the QR and code; People shows nothing but an Invite button when nobody is visiting; a pane''s share chip offers Share this pane and Share more; no Share this pane window exists anywhere', sign_off: none, effort: medium, stakes: rework, blast: capability}
source: owner, 2026-09-24, Claude Code session in Relay
links: {plans: [], commits: [0dd9c980, e4b67044], evidence: [docs/qa_evidence/2026-09-24-smdx-sharing-pane/], related: [SHRP, A11T, PRM2, FR1C, W5N2], github: null}
---
...
- A pane's share chip offers exactly "Share this pane…" (People, form open on that pane) and "Share more…" (People, picker). "Pair a phone…" opens Devices with a pairing offer started; leaving Devices or pressing Done revokes the code.
- Failure looks like: a dialog still opening from any entry point; quiet panes listed on People; a pairing code minted on opening the pane rather than on "Add a device…"; a knock or a device ask not surfacing on the right page.
...
## Tests
`ctest --test-dir build -R '^sharing$'` (28 cases: model + both pages)
`ctest --test-dir build -R '^remotesettings$'`
`ctest --test-dir build -R '^remotepane$'`
`manual: docs/qa_evidence/2026-09-24-smdx-sharing-pane/`
...
## Try it
1. Restart Relay so the new binary runs. Click the share button under any pane's prompt box: a two-row menu, "Share this pane…" and "Share more…". The first opens the Sharing pane on People with the invite form already on that pane; the second with the picker (panes by tab, the tab, Everything).
2. Plug menu › Pair a phone…: the Sharing pane opens on Devices with the QR, the typed code and Copy link above your paired devices. Press Done: the code is withdrawn (the phone says the code is no longer valid).
3. Pair a phone through the code: the approval card appears at the top of Devices, Refuse has the focus, the tab reads "Devices · 1 asking".
4. Make a link for a pane, open it in a browser, knock: People shows "Waiting for you" and the tab reads "People · 1 waiting"; after admitting, "Shared now" lists that pane only.
5. No window titled "Share this pane" opens from anywhere (chip, pane menu, plug menu, palette `pane.share` / `remote.pair`, Options › Remote › Pair…).
