...
- [x] Live tail of the running guest's transcript: the session is named rather than guessed (the harness for Tier A, `program_state.guest_session` for Tier B, a hook's or the rollout tail's own id for a guest the user started), polled on the ticks that already exist <!-- t:t1 -->
