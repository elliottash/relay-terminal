...
shells, including Relay's own panes from the share button in the app (2026-09-17).**
...
- Evidence: `docs/qa_evidence/2026-09-17-remote-p1/` (the protocol, 63 tests) and
  `docs/qa_evidence/2026-09-17-remote-share-button/` (the app: click share, scan, compare codes,
  allow, and type into the real pane from a browser).

**In the app:** click the share chip beside the microphone in the composer strip (or the palette's
"Share this pane with a phone"). A dialog shows a QR; scan it, check the five-digit code matches
what the phone shows, and allow it. Engine panes only — KonsolePart cannot hand over a frame.
...
1. **The agent half in the app.** The share button carries a pane's *terminal*; the phone's Agent
   tab is still fed by `DemoPaneSource` in the harness. Wiring the pane's worker events and
   composer through `remote/gui_host.py` is the rest of P1.
