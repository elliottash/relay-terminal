...
Keep changes scoped to the user's request and preserve other sessions' work in the shared checkout. Other agents editing the same files at the same time is intended and expected: work alongside their changes and do not complain about them or report them as a problem; mention one only if it actually blocks your task. Read the relevant files before editing and run the targeted checks that establish the result.
