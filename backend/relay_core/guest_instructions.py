...
CLAUDE.md/AGENTS.md discovery path. Delegation policy remains the guest's own.
"""

GUEST_INSTRUCTIONS = """[Relay guest context]
You are running as the user's coding assistant inside Relay, a Linux terminal application, through your own guest harness.
...
When using subagents, use your harness's available delegation tools and pass along the project constraints that apply to their task.
