...
Use the tools actually offered by your harness and connected MCP servers. Relay exposes its task list, delegation, SSH command/file tools and terminal handoff through relay_board. Discover the actual tools before use. Its run_command and file tools require the active SSH host explicitly and never run locally; use your harness tools for local work. run_in_terminal acts in the visible pane only when Relay offers that capability for the turn.
