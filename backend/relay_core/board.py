...
def note_text() -> str:
    """The marked one-liner that points an instruction file at `RELAY.md` (#C8XD).

    An `@` import, not prose: `instructions.load` resolves it, and so do Claude Code and Codex,
    so the Board policy reaches every prompt instead of being described to it.
    """
    return f"""{POINTER_START}
...
def relay_md_text(board: "Board") -> str:
    """A project's first `RELAY.md` (#C8XD): a short starter for the owner's own guidance with
    the generated Board section at the end.  Everything outside the markers is the owner's."""
    try:
        name = board.repo.resolve().name
    except OSError:                                       # pragma: no cover - vanished cwd
        name = "This project"
    return f"""# {name} — agent guidance

One line on what this project is, in your own words. Everything in this file is yours to edit;
Relay only ever rewrites the generated Board section at the end.

## Working rules

- How you want agents to work here: commit style, test commands, review expectations.
...
def pointer_files(board: "Board") -> list[tuple[str, str]]:
    """The instruction files that need a generated block, as (path, whole new content) pairs.
    ...
    if not relay_md.exists():
        out.append((str(relay_md), relay_md_text(board)))
    else:
        try:
            new = _with_pointer(relay_md.read_text(encoding="utf-8"), pointer_text(board))
        except (OSError, UnicodeDecodeError):              # pragma: no cover - unreadable file
            new = None
        if new is not None:
            out.append((str(relay_md), new))
    note = note_text()
    for name in POINTER_TARGETS:
        ...
        if not path.is_file():
            continue
        try:
            new = _with_pointer(path.read_text(encoding="utf-8"), note)
        except (OSError, UnicodeDecodeError):              # pragma: no cover - unreadable file
            continue
        if new is not None:
...
def policy_files(board: "Board") -> list[tuple[str, str]]:
    """`POLICY.md` when it is missing or stale, plus `RELAY.md` and the files that need the note."""
    out: list[tuple[str, str]] = []
