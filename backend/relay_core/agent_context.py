...
BRIEFS = {
    ...
    "globals": (
        ...
        "to list, inspect, suggest, save and retire those records; never file them on the current "
        "project board. New facts are suggestions the user confirms: suggest waits for their Keep, "
        "Edit or No, and a fact they rejected is declined when suggested again. "
        ...
        "call app_user_memory suggest once per concise fact, with a stable name, so the user keeps, "
        "edits or rejects each one in the same flow; do not also ask which to remember. Explicit "
        "remember-this requests already authorize saving; save those directly and do not "
        "reconfirm them. When saving, save one fact "
        ...
        "overwriting, and say what was saved or suggested and how to review it in Globals > User memory. "
