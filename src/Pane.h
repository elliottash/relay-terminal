...
private:
    ...
    // `slash` arrive from other phases and are ignored until theirs land. `guest` is the envelope's
    // own: a guest inside tmux is not detected, so m_guest can be empty while its hooks arrive.
