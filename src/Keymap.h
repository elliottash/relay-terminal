...
private:
    Keymap() {
        ...
        add("ssh.remoteSessions", "tab", "Persistent sessions…: the holder sessions this pane can reach — on its host over ssh, or on this machine — reattach one in a new tab, or end it", {});
        ...
        add("pane.closeEndRemote", "pane", "Close pane and end its holder session, remote or local", {});
        ...
        add("background.open", "pane", "Board: background work (again to close it)", {QStringLiteral("Ctrl+Shift+B")});
        ...
        // is nested under Sessions. Background work has its own Board tab. agent.resume and conversations.open remain
        // registered for slash commands and user keybindings.json overrides.
        add("sessions.open", "pane", "Sessions: saved sessions and recently closed (again to close it)",
        ...
        add("terminal.interrupt", "terminal", "Interrupt the running command (Esc when only the shell runs, Alt+Esc anytime)",
            {QStringLiteral("Alt+Esc")});
        ...
        add("agent.stop", "agent", "Stop the agent turn (Esc; Alt+Esc is the shell's)", {});
        ...
        // this pane's subagent pane; from the subagent pane it closes it and returns to the main agent
        // (Esc there holds the live agent in front first, #ZQNG). A for agents; no preset binds
        // Alt+A, and Readline leaves M-a unbound.
        add("agent.subagentPane", "agent", "Subagents: open this pane's subagent tabs, or close the active subagent pane (a running agent in it is held first)",
        ...
        // tab, on `agent.modelOptions`; the vscode preset's Ctrl+Alt+. went with it.
