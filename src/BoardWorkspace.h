...
#pragma once
// Which project's Board a pane is looking at.
//
// The Board is a per-project board: its cards are the files under the project's board folder —
// `board/` on a board made from 2026-09-21 on, `.switchboard/`, `switchboard/` or `issues/` on an
...
// called, so a project with an old `issues/` board inside a tree whose root has a `board/`
