...
// relay::links: which parts of terminal output are files, folders and URLs, what they
// resolve to, and the keyboard cursor that steps through them.
//
...
// - src/main.cpp: routing an activated link to a Relay explorer or preview pane.
//
// Issues: issues/features/needs_qa_llm/2026-09-17-clickable-paths.md (#YZTK) and
// .../2026-09-17-keyboard-jump-to-output-links.md (#GWXM).
#pragma once
...
struct Candidate {
    ...
    QString path;      // Path only: the span without the :line:column suffix
...
struct Target {
    bool valid = false;
    Kind kind = Kind::Path;
    QString target;         // the URL as written, or the cleaned absolute path
...
// ids and URLs; skips `--flags`, bare numbers and version strings.
QVector<Candidate> candidates(const QString &text);

// `candidate` against a pane's directory. `cwd` resolves relative paths, `home` expands
// `~`. A path that the probe does not find is not a link (`Target::valid` stays false).
