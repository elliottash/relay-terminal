...
@dataclass
class Room:
    """One pairing opportunity: single use, short-lived."""
    room: str
    ...
    def check(self, secret: bytes) -> bool:
        """Constant-time check, single use. A failed attempt counts; five burn the room."""
        if self.expired:
            return False
        self.attempts += 1
        if not hmac.compare_digest(self.secret, secret):
            ...
            return False
        # Spent whether or not the user goes on to allow the device: a second proof must fail.
        self.spent = True
        return True
