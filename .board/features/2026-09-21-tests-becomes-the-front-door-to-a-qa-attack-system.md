...
## Discussion points
### Thread summary (2026-09-23)

**Idea.** Keep **Tests** as the entry point, and add a broader QA workflow in which agents try to discover defects in a project. Keep known test results separate from exploratory findings. The intended product is a short path from a suspected bug to a reproducible, reviewable counterexample.

**Research recommendation, not an approved scope.** Design for projects opened in Relay, with Relay as the first pilot. Start on demand and advisory: select a frozen change, investigate Relay's test-run lifecycle, inject a bounded child-process fault, replay it against agreed expectations, and let a person adjudicate the finding. A confirmed finding can become a card and regression test; repair and verification follow separately. The alternative first pilot is scanner fuzzing if native crash discovery is the priority.

**Why start narrowly.** Relay already has Tests history, jobs, agents, and a board handoff, but needs durable attack/finding records, complete replay manifests, containment, and scheduling before recurring campaigns. A model's allegation or agreement between models does not establish a defect. Measure confirmed new defects, false reports, duplicates, replay success, cost, and human triage time on held-out planted/historical cases and ordinary changes before considering gates. Provenance's W7-CRITIC supplies a useful evaluation protocol; completed reliability results were not found. Mutation testing measures test strength, and its schedule should follow measured cost rather than an assumption that it is always too slow per change.

**Decisions still with the owner.** Confirm general-project scope or Relay-only tooling; choose lifecycle/replay or crash-focused fuzzing as the first pilot; agree the expected lifecycle behavior, execution limits, budget, and who adjudicates disputed requirements. No attack or implementation was authorized by the research discussion.

[Detailed research report](../../reports/Relay%20QA%20attack%20system.md) · [Supporting research notes](../../research_notes/Relay%20QA%20attack%20system/) · [Earlier QA synthesis](../../docs/QA-ACROSS-FIELDS-RESEARCH.md).
