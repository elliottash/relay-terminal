...
## Plan
**Goal.** Measure the author x reviewer review matrix on CodeJudgeBench and answer the owner's question — for Claude-made bugs, is Kimi or GLM the better reviewer — then keep or change `VERIFIER_RANK`'s ordering with the reason recorded on #T71W. The design, cost and power are settled on this card and in `docs/qa_evidence/2026-09-19-cross-provider-qa/proposed-study-design.md`; this plan is the run book. **Run is the go-ahead for the 2x2 arm only (~4,850 calls, ~2,020 of them metered on the Kimi key); the 4x4 (task a7) waits for a second go with the 2x2 in hand.**
...
5. **Repeats.** 3 repeats on a random 100-item subset, both reviewers, both responses (+800 calls) — the noise floor that caps any correlation this data can show.
...
**Risks.**
- **The spend is the gate** (this card's one `waiting_on: owner`). Run authorises ~4,850 calls as budgeted above; if the owner wants the Gemini/Qwen contrast arms now instead of after the 2x2, that is a different authorisation — ask before extending.
