...
## QA checklist
Verified 2026-09-25 by a verifying session at rev `2db966438ab8bc61e0f9f1ff89a51853286ad0f8` (pinned during a moving HEAD; all eight claimed commits are ancestors). C++ suites built fresh in a clean detached worktree so no other session's uncommitted work is in the binaries. Evidence: `docs/qa_evidence/2026-09-25-verify-RND7/` (NOTES.md) plus live screenshots from this sweep's drive.

**Done means:**
- Equal-rank entries configurable for main/planning/helper/subagents, model+effort paired — **passed** (code + UI: `docs/qa_evidence/2026-09-25-verify-N4PW/12-narrow-order.png` shows rank / Tie with above; `16-keyboard-tab.png` shows the per-job override table; `tiedRanksAndPlanningListAreVisible` unit test green).
- Fresh lifecycle draw chooses among best-rank eligible entries and keeps the choice for later turns — **passed by test evidence** (`relay-modelcatalog-tests` 73/0 including the draw/fallback cases; backend `tests.test_roles` + `tests.test_subagents` 138 OK — deeper manual lifecycle exercising not re-driven live this pass).
- Subscription usage weights within the top rank; unknown usage neutral; exhausted skipped — **passed by test evidence** (same suites; the usage-state machinery is #JX8Z's, whose scores refresh code sits in `src/ModelCatalog.cpp`).
- Legacy lists keep their current ordering — **passed** (migration covered in modelcatalog tests; the live drive's Enabled tab preserved an existing list across restarts).

**Tests, line by line:**
- Exact-tree build per landing commit — **not re-run per-commit**; superseded by one clean-worktree build of the four test targets at `2db96643`: green.
- `PYTHONPATH=backend python3 -m unittest tests.test_roles tests.test_subagents` — **passed**: 138 tests OK (card recorded 104; coverage grew).
- `build/relay-modelcatalog-tests -silent` — **passed**: 73/0 (card: 70).
- `xvfb-run -a build/relay-jobstab-tests -silent` — **passed with a note**: 23/1; the one failure is the #E8V1 rename fallout at `tests/jobstab_test.cpp:181` (filed as #SYTR), not this change.
- `xvfb-run -a build/relay-modelspane-tests tiedRanksAndPlanningListAreVisible -silent` — **passed**: 3/0.

Unresolved: nothing for this card. The #SYTR stale-test card covers the only failure seen.

Reviewed 2026-09-25 by the verifying session (qa-verify-RND7), rev `2db96643`.
