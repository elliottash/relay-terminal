...
created: '2026-09-24'
verify: {artifact: visual, primary: script, also: [probe, person], human: required, criteria: 'on a fresh XDG_CONFIG_HOME the window opens with the terminal left and the Start pane right, no dialog appears, twelve recipes are listed, each recipe''s Enter produces its first outcome without an error (a staged turn for the agent recipes, a connected persistent session with two panes for the ssh one), Esc leaves only the approvals pick and the pane never returns after it', sign_off: none, effort: high, stakes: reputation, blast: capability}
...
- The twelfth task, "Start a persistent remote session with multi-pane layout" (owner, 2026-09-24): a host row reusing `ssh.connectPersistent`'s saved-or-recent picker, then the persistent connect itself (mosh + `zellij attach --create relay-<user-host>`, `ssh -t` and tmux fallbacks as shipped), opening the session with a multi-pane layout — a shell pane and an editor pane — so the panes live on the host, survive a disconnect and reattach on a second run. Needs: an ssh target (mosh on this machine for roaming; without it the ssh fallback is stated in the needs line). Nothing is installed on the host; zellij's one-attach-at-a-time rule means the panes are inside the session, not a second Relay attach.
...
- Persistent remote sessions: "Connect to host (persistent)…" (`ssh.connectPersistent` → `openSshMenu(true)`, `src/RelayWindowCore.cpp:240`; palette item and saved/recent host picker `src/RelayWindow.cpp:481-489`) runs mosh + `zellij attach --create relay-<user-host>` with `ssh -t`/tmux fallbacks; a multi-pane layout rides zellij inside the session (`docs/SSH-AND-MOSH.md`, "Persistent connects") — zellij refuses a second concurrent attach, so the panes are zellij panes, not a second Relay attach.
...
### Steps
1. **Recipes as data.** `backend/relay_core/start_recipes.py` loads `backend/relay_core/recipes_bundled/<id>.json` (id, title, one-line blurb, `needs_folder`, `layout`, `prompt` with `{folder}`/`{host}` slots, `requires` in the task-plugin vocabulary, `plugin`, `skill`, `arms` hint ids, per-platform overrides for paths and programs) and answers a `start_recipes` protocol request with each recipe's status on this machine: `ready`, or `needs` with the one-line fix. An installed skill or task plugin may declare a recipe of its own. Twelve bundled recipes: the eleven of §3.3 ("Triage my email" and "Organize multiple subscriptions" say `needs a skill` until #SZ1H; "Generate some art" is satisfied by a stored OpenRouter key **or** the Relay Free gateway serving images, per the note below) plus the owner's 2026-09-24 addition, **"Start a persistent remote session with multi-pane layout"**: no folder, a `{host}` slot filled by the saved-or-recent host picker, `action: ssh.connectPersistent` with a zellij layout of a shell pane and an editor pane named in the attach command, `requires: an ssh target` (mosh present → roaming, absent → the `ssh -t` fallback stated in the needs line), and reattach on a second run. Protocol section added to `docs/AGENT-SESSIONS-PROTOCOL.md`. Tests: `tests/test_start_recipes.py`.
...
- The ssh recipe must not install anything on the host or open a second attach to one zellij session; it stages the one persistent connect and the layout rides inside it.
