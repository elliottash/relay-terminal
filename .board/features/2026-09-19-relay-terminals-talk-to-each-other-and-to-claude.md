...
## Done means
- With two agent panes open, the agent in one can call `pane_list` and see the other by `p<n>` handle and title, and `pane_send` returns at once with `delivered` or a named refusal (`unknown_pane`, `self`, `closed`, `not_configured`, `cap`, …) — never a wait.
- A message to an idle pane starts a turn there with nobody approving it, framed as another model's words; a message to a busy pane is read at its next step boundary. Failure shows as the message vanishing, the idle pane staying idle, or the text arriving unframed as if the user typed it.
