...
links: {plans: [], commits: [e654c416f1e1db2c8c512a56bbdf812fc80a0143], evidence: [docs/qa_evidence/2026-09-21-phantom-plan-model-PMX7/README.md, docs/qa_evidence/2026-09-25-verify-PMX7/], related: [HR5E], github: null}
---
...
## QA checklist
Verified 2026-09-25 by a verifying session at rev `2db966438ab8bc61e0f9f1ff89a51853286ad0f8` (clean worktree). Evidence: `docs/qa_evidence/2026-09-25-verify-PMX7/`.

**Done means, item by item (all migration-behavior lines):**
- One-time retirement of the legacy `roles/planning/*` override; explicit Jobs overrides survive; terminal + console agents share the migration; shell commands never take the plan route; regressions against legacy survival, repeat migration, and stale UI copy — **passed by test evidence**: `tests.test_roles` + `tests.test_plan_turns` fully green at HEAD (the card's named regression suites; count grew past its 110). A live upgraded-installation drive was not repeated this pass — the suites construct exactly that seeded-config scenario.

**Tests, line by line:**
- `relay-jobstab-tests` / `ctest -R '^jobstab$'` — **passed with a note**: 23/1; the 1 is the #E8V1 rename fallout (`tests/jobstab_test.cpp:181`, #SYTR), unrelated to the plan route; the card's 1/1 pass predates the rename.
- `PYTHONPATH=backend python3 -m unittest tests.test_roles tests.test_plan_turns` — **passed** (OK).
- `scripts/relay-build --target relay` — **passed** (this sweep).
- `relay-board.py check` — this card validates (its pre-existing foreign-card errors are still visible board-wide; none here).
- Implementer's README — **present**.

Unresolved: nothing for this card.

Reviewed 2026-09-25 by the verifying session (qa-verify-PMX7), rev `2db96643`.
