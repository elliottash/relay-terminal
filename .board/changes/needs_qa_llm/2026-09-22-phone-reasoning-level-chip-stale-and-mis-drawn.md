...
links: {plans: [], commits: [b8222865, 44a46ca1, 2ee01f0c, d329d88a, a0882679, 9c1b1d18, 7d313776], evidence: [docs/qa_evidence/2026-09-22-phone-ux-drive/, docs/qa_evidence/2026-09-22-streamA-pane/, docs/qa_evidence/2026-09-22-streamE-effort/, docs/qa_evidence/2026-09-25-verify-EFT9/], related: [MDL1, PH0N], github: null}
---
...
## QA checklist
Verified 2026-09-25 by a verifying session at rev `2db966438ab8bc61e0f9f1ff89a51853286ad0f8` (clean worktree). Evidence: `docs/qa_evidence/2026-09-25-verify-EFT9/`. All seven commits are ancestors; later cards renamed the two chip tests (names tracked below).

**Done means, item by item:**
- The chip always says the level the pane is on; a level-only `pane_state` repaints it — **passed by test evidence** (`test_a_level_only_state_repaints_the_chip_and_the_closed_chip_has_no_tick`, green at HEAD under its renamed name).
- A refused pick is answered by the pane's own state; a fixed level draws an unchangeable chip — **passed by test evidence** (`test_a_model_whose_level_is_fixed_draws_a_chip_that_cannot_be_picked_from` green; the refusal republish and `effort_fixed`/reason end-to-end cases in `tests.test_remote_pane_state`, OK at HEAD).
- Closed chip reads `high` — no checkmark, chevron in its own box — **passed by test evidence + code** (app/pane.js:311 separate `rp-effort-chevron`, :1170 renderEffort outside the model-signature guard, :1207 fixed handling; the card's measured headless-Chrome captures remain the visual evidence).

**Tests, line by line:**
- `python3 -m unittest tests.test_pane_view` — **passed with a note**: this card's chip tests pass (renamed by later cards — old names no longer exist); one unrelated class (OutboxTests) errors on `app/outbox.js` module-format drift, third instance of #DX4A's family (noted on that card's thread).
- `python3 -m unittest tests.test_remote_pane_state` — **passed** (OK).
- `ctest -R panestate` — **stale**: no such ctest at HEAD (`relay-panestate` is a static library, CMakeLists.txt:422); coverage lives in the suites above.
- The three manual evidence dirs — **present**.

Unresolved: nothing for this card; the refusal path still has no C++ test (the card itself flags this — unchanged at HEAD).

Reviewed 2026-09-25 by the verifying session (qa-verify-EFT9), rev `2db96643`.
