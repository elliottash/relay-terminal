...
- 2026-09-24, owner: "all recs approved, go ahead and file the three delivery cards linked to #9HS0." Questions 5–19 are settled as recommended (thread). Delivery cards: #6VMF (Start pane and recipes), #CWVF (prompt box and Relay Free labelling), #X3SM (behaviour-triggered tips and the wildcard preview). Same message: "add the cheap openrouter image generator to relay free" is #Z82M, which removes question 8's "needs a provider" line on a keyless install once deployed.
- 2026-09-24, owner: "can you add "start a persistent remote session with multi-pane layout" as one of the tasks." The starter-task list becomes twelve. The recipe is specified on #6VMF: it reuses the shipped persistent connect (`ssh.connectPersistent`, mosh + zellij with `ssh -t`/tmux fallbacks) and adds a two-pane zellij layout inside the session, so the panes live on the host and survive a disconnect; needs: an ssh target.
