...
<!-- relay:entry 20260926T040921Z-v3 author=claude-code kind=progress -->
landed c8dce2511c2f: the actionable handoff kind is now a function of every outbox row a job has (stable across polls and restarts: a `failed` row delivered before a crash and `author_required` after it still wake the author once; other rows are history); queue-mode Board snapshots are recorded synchronously before the turn completes (agent._sync_board_writes keeps only the legacy board-sync on its daemon thread). Tests: split delivery with a restart, durability without the legacy runner.
