...
<!-- relay:entry 20260925T223204Z-1u author=agent kind=note -->
Verification (rev 2db96643): PASSED with one gap — row on every (now five) tabs, Alt+Q opens, no second row in Sources, models brief tests green (live screenshots in docs/qa_evidence/2026-09-25-verify-MH7P/). The card's own C++ test is stale on #E8V1's renamed strings (#SYTR), which also blocks its fold/spec assertions from running; the fold-back click itself was not landed live this pass. Moved needs-verification → needs-qa-llm.
