...
<!-- relay:entry 20260920T052900Z-c4 author=agent kind=evidence model=kimi-k2.7-code-highspeed -->
- ✦ landed in 284aa746 on main: the notification post (`onTurnEnded` → "Plan ready/failed: #ID", source `board:<workspace>#<id>`, no focus move), `openNotificationSource` with cross-window board reveal, `notifications.jump` (Ctrl+Shift+1) with the ~4 s walk and single-entry `markSeen`, the `notifications.jump.mouse` hint, `markSeenOneEntry` in `tests/notifications_test.cpp`, and the ARCHITECTURE.md section. land.py held the commit until the exact tree built; the working tree carried four other sessions' uncommitted hunks in the same files (a board-split floor, a card-delete UI, chrome hairlines) and every one of them was picked out by content and left uncommitted for its own session. The card moves to needs-verification with `docs/qa_evidence/2026-09-20-board-plan-notification/`.
