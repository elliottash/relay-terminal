...
<!-- relay:entry 20260925T221849Z-ss author=agent kind=note -->
Re-verification sweep 2026-09-25 (verifying session): this card was already verified on 2026-09-22 (QA checklist + Verdict above; item 11 paired-phone check remains blocked — no phone in the QA rig — so per that verdict the card stays in needs-verification; not moved). Refresh at rev 9a13cfe2 (clean worktree, HEAD moved on from 54018502 during the sweep): the named C++ tests pass except `conversations` (theHelperConsoleSitsAtTheBottomCollapsedAndAsksAsTheSessionsPane, tests/conversations_test.cpp:2439) which fails on the "Helper Agent" → "Agent" rename from #E8V1 (e6febb9f), and the named python files no longer run green at HEAD: 4 failed + 6 errored (test_web_model_name: app/modelname.js no longer offers ESM named exports after 1aa4d54a; test_presets GuiMirror: src/Pane.h drift; test_keybindings GuiDefaults: default-key drift). All regressions postdate and are unrelated to this card's change; filed as separate bug cards.
