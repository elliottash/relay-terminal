...
// every escape sequence except CSI SGR and the OSC 8 links of inline image rows and
// prose blocks.
#pragma once
...
// of inline image rows (engine/core/InlineImage.h, card #1MGS) and of prose blocks (kProsePrefix,
// card #MTCS) and no other link, so a restored pane draws its pictures again and re-wraps its own
// word-wrapped output. `linkUri` resolves a cell's link id as for lineToAnsi().
...
// its URI is an image row's (inlineimage::parseImageUri) or a prose block's (kProsePrefix; a
// fragment-carrying label URI rides the same prefix, card #MDKN), so the most a hand-edited
// file can do is make the view paint a local image file — a prose URI is inert paint unless
// the pane also holds a block registered under it. Every other escape introducer and control is
