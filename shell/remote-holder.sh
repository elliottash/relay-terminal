...
# session name; $2 is an optional start directory, used only when it is one.
d=${XDG_CACHE_HOME:-$HOME/.cache}/relay
if command -v tmux >/dev/null 2>&1; then mkdir -p "$d" && echo set -g status off > "$d/tmux.conf" && echo set -g prefix None >> "$d/tmux.conf" && echo set -g prefix2 None >> "$d/tmux.conf" && echo set -g mouse off >> "$d/tmux.conf" && echo set -s set-clipboard on >> "$d/tmux.conf" && echo set -g allow-passthrough on >> "$d/tmux.conf" && echo set -g window-size latest >> "$d/tmux.conf" && echo set -g history-limit 50000 >> "$d/tmux.conf" && echo set -s escape-time 10 >> "$d/tmux.conf" && echo set -g focus-events on >> "$d/tmux.conf" && { [ -d "$2" ] && exec tmux -L "${RELAY_HOLDER_SOCK:-relay}" -f "$d/tmux.conf" new-session -A -D -s "$1" -c "$2"; exec tmux -L "${RELAY_HOLDER_SOCK:-relay}" -f "$d/tmux.conf" new-session -A -D -s "$1"; }; fi
